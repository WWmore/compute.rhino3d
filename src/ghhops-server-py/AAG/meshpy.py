...
class Mesh(object):
    ...
    def face_ring_faces_iterators(self, sort=False, order=False,return_lengths=False):
        "Hui add"
        H = self.halfedges
        if order:
            i  = self.vertex_ring_ordered_halfedges()
            ...
            i  = np.intersect1d(np.where(H[:,1] >= 0)[0],np.where(H[H[:,4],1]>=0)[0])
            f = H[i,1]
            fj = H[H[i,4],1]
            if sort:
                i  = np.argsort(f)
                ...
                fj = fj[i]
        if return_lengths:
            i  = np.ones(fj.shape[0], dtype=np.int)
            lj = utilities.sum_repeated(i,f)
            return f, fj, lj
        else:
            return f, fj
    ...
    def face_triangles(self):
        ...
        while len(f) > 0:
            ...
            offset += len(f_i)
        T = np.array(T,dtype=int) # Hui add
        return T, face_index
    ...
    def vertex_normals(self):
        "note: mean of neighbouring (not unit) face normals"
        N = self.face_vector_areas()
    ...
    def bounding_box(self,diameter=False): # Hui add diag
    ...
    def extended_shape_operator(self, area_normalization=False, use_sine=False):
        "Hui note: theory from <9.21-Curvatures of polyhedral surfaces.pdf>"
        v, ej = self.vertex_ring_edges_iterators(sort=True)
        v1, v2 = self.edge_vertices()
        if use_sine:
            B = self.edge_sine_vectors()
        else:
            B = self.edge_angle_vectors() # Hui note: use angle as a weight
    ...
    def edge_cotangents_weigths(self): # Hui: for triangle mesh
        H = self.halfedges
    ...
    def mean_curvature_normal(self): # Hui: for triangle mesh
        b = self.boundary_vertices()
    ...
    def catmull_clark(self, steps=1,is_fixcorner=False): ##Hui add: fix_corner
        for i in range(steps):
            self._catmull_clark(is_fixcorner)
    ...
    def _catmull_clark(self, is_fixcorner=False): ##Hui add: fix_corner
        V = self.V
        E = self.E
        H = self.halfedges
        ...
        if is_fixcorner:
            _,_, valence = self.vertex_ring_vertices_iterators(return_lengths=True)
            ic = np.where(valence==2)[0]
            old_vertices[ic] = self.vertices[ic]
    ...
    def delete_edge(self, edge_index):
        ...
        if len(self.halfedge_ring(h)) < 3:
            return False
        if len(self.halfedge_ring(self.twin(h))) < 3:
            return False
        self._open_trash()
    ...
    def vertex_multiple_ring_vertices_faces(self,vertex_index,depth=1):
        "Hui: v--->ring--->face_order"
        face = self.faces_list()
        vi, vj = self.vertex_ring_vertices_iterators()
        vk, fj = self.vertex_ring_faces_iterators()
        ring = np.array([], dtype='i')
        search = np.array([vertex_index], dtype='i')
        order_faces = []
        for i in range(int(depth)):
            ...
            for v in search:
                vf = []
                for j in np.where(vk==v)[0]:
                    order_faces.append(fj[j])
                    vf.append(face[fj[j]])  
                vf = np.unique(vf)
                vring = np.hstack((vj[vi == v], vring, vf))
            vring = np.unique(vring)
            vring = vring[np.invert(np.in1d(vring, ring))]
            search = vring
            ring = np.hstack((ring, vring))
            if len(ring) == self.V:
                return np.unique(ring), np.unique(order_faces)  
        return np.unique(ring), np.unique(order_faces)    
