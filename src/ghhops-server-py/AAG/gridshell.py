...
from quadrings import MMesh # Hui

try: 
    from constraints_basic import columnnew,con_laplacian_fairness,\
        con_fair_midpoint,con_bigger_than
except:
    pass
#------------------------------------------------------------------------------
"""Hui add: 
...
class Gridshell(MMesh):  # Hui
    ...
    def con_fairness(self,move,N,poly_edges,
        ...
        cv = columnnew(vvv, move,Vnum)
        ca = columnnew(vva,move,Vnum)
        cb = columnnew(vvb,move,Vnum)
        col = np.r_[cv,ca,cb]
    ...
    def con_selected_polylines_fairness(self,move,N,poly_edges,**kwargs):
        ...
        cv = columnnew(vp,move,Vnum)
        ca = columnnew(vpl,move,Vnum)
        cb = columnnew(vpr,move,Vnum)
        col = np.r_[cv,ca,cb]
        data = np.r_[2*one,-one,-one]
        K = sparse.coo_matrix((data,(row,col)), shape=(3*num, N))  
        s = np.zeros(K.shape[0]) 
    ...
    def con_avoid_shrinkage_edge_length(self, **kwargs):
        ...
        v1, v2 = self.edge_vertices()
        c_v1 = columnnew(v1,0,self.V)
        c_v2 = columnnew(v2,0,self.V)
