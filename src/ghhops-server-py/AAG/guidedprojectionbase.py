...
class GuidedProjectionBase(object):
    ...
    iterations = 1 # Hui note: =each running how many optimiztions+print
    ...
    _X = np.array([]) #None ##Hui note: change from None to np.array([])
    ...
    _r0 = np.array([]) #None ##Hui note: change from None to np.array([])
    ...
    _s0 = np.array([]) #None ##Hui note: change from None to np.array([])
    ...
    _iterative_constraints = [] #hui comment: no use in all files
    ...
    # @weights.setter
    # def weights(self, weights): # Hui add
    ...
    def initialization(self): # Hui note: for all initial_reset
        self._initialization()
        self.reinitialization()

    def _reinitialization(self):
        self.on_reinitialize()
        if len(self._X)!=0: # Hui add this if-else
            pass
        else:
            self._X = None
        self._X0 = None
    ...
    def reinitialization(self):
        ...
        #self._r0 = np.array([]) # Hui add
        #self._s0 = np.array([]) # Hui add
    ...
    def set_weight(self, name, value):
        if name in self._weights:
            self._weights[name] = float('%0.2f'%value)
        else:
    ...
    def _out_error(self, name):
        ...
        if error is None:
            ...
            "mean | max | weight"
            string = '.{:>22}: {:>11.4E} | {:>10.4E} | {:>5.1E} .\n' #Hui: from {:>10.4E} to {:>5.1E}
    ...
    # Hui note: if such below are defined/replaeced in gp.py, then can comment
    #           if recall by opt_main, then can't comment.
    # def build_iterative_constraints(self):
    #     pass
    
    # def build_iterative_fairness(self): # Hui comment: no use here.
    #     pass
    ...
    def build_constant_constraints(self):
        "Huinote: if guidedprojection.py, this fun. is skipped"
        self.weights['N'] = self.N
        H, r =self.mesh.constant_constraints(**self.weights) #from gridshell.py
        self.add_constant_constraint(H, r, 'mesh_constant')
        # print(self.weights.get('fixed_vertices'))
        #print(self.weights.get('self_closeness'))
        # print(self.weights.get('reference_closeness'))

    def build_constant_fairness(self):
        "Huinote: if guidedprojection.py, this fun. is skipped"
        self.weights['N'] = self.N 
        K, s = self.mesh.fairness_energy(**self.weights) #from gridshell.py
        self.add_constant_fairness(K, s)   
        ##print(self.weights['mesh_fairness'])
    # -------------------------------------------------------------------------
    #                                Build
    # -------------------------------------------------------------------------
    ...
    def _build_constant_matrices(self):
        ...
        self._s0 = np.array([])
        self.build_constant_constraints() # fun. from guidedprojection.py
        self.build_constant_fairness() # fun. from guidedprojection.py
    ...
    def _build_iterative_matrices(self):
        ...
        self._r = np.array([])
        self._s = np.array([])
        self.build_iterative_constraints() # fun. from guidedprojection.py
        #self.build_iterative_fairness() # no use 
    ...
    def optimize(self):
        ...
        for i in range(self.iterations):
            ...
            #print(H.shape,r.shape,K.shape,s.shape,X.shape)
